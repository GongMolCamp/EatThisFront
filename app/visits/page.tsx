  ...
  return (
    <div className="flex items-center gap-4 border-b border-gray-300 p-4">
      ...
      <div className="flex flex-col flex-1">
        ...
        <p className="text-gray-500 text-sm">{new Date(visit.createdAt).toLocaleString()}</p>
      </div>

      {/* Status Buttons */}
      {!editing ? (
        <button onClick={() => setEditing(true)} className="border border-[#4A90E2] text-[#4A90E2] px-4 py-2 rounded-md">
        ...
        <div className="flex gap-2">
          <button onClick={() => handleUpdateStatus("NOT_VISITED")} className={`px-4 py-2 rounded-md ${status === "NOT_VISITED" ? "bg-[#FFA726] text-white" : "border border-[#FFA726] text-[#FFA726]"}`}>
            미방문
          </button>
          <button onClick={() => handleUpdateStatus("LIKE")} className={`px-4 py-2 rounded-md ${status === "LIKE" ? "bg-[#FFA726] text-white" : "border border-[#FFA726] text-[#FFA726]"}`}>
            좋아요
          </button>
          <button onClick={() => handleUpdateStatus("DISLIKE")} className={`px-4 py-2 rounded-md ${status === "DISLIKE" ? "bg-[#FFA726] text-white" : "border border-[#FFA726] text-[#FFA726]"}`}>
            싫어요
          </button>
        </div>
      )}
